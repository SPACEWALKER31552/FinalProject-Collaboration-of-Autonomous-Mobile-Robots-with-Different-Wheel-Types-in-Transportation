...
int main(int argc, char **argv)
{
	...
	while (nh.ok())
	{
		...
		if (angz > 0.1 && linx != 0 || angz > 0.1 && linx == 0) //> 0.12
		{
			...
			follower1velocity.angular.z = signI * 1.8; // 1.37*uleader;
			...
			else if ((slavezeta - startz) < 0 && (slavezeta - startz) >= -90)
			{
				...
				follower2velocity.angular.z = angz / 8;
			}
			...
			else if ((slave2zeta - start2z) <= 90 && (slave2zeta - start2z) >= 0)
			{
				...
				follower3velocity.angular.z = angz / 6.3;
			}
		...
		else if (linx != 0)
		{
			...
			follower1velocity.linear.x = 1.5 * linx; //1.5
