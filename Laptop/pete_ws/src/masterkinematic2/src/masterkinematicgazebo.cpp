...
int main(int argc, char **argv)
{
	...
	{
		ros::spinOnce();
		if (angz > 0.10 && linx != 0 || angz > 0.10 && linx == 0) //> 0.12
		{
			if (startz == 0)
			{
				startz = slavezeta;
			...
			{
				follower2velocity.linear.x = linx - (slaveturnjcomponent * abs(cos((int(abs(slavezeta - startz)) % 90) * 3.14 / 180))) - (slaveturnicomponent * abs(sin((int(abs(slavezeta - startz)) % 90) * 3.14 / 180)));
				follower2velocity.linear.y = -(slaveturnicomponent * abs(cos((int(abs(slavezeta - startz)) % 90) * 3.14 / 180))) + (slaveturnjcomponent * abs(sin((int(abs(slavezeta - startz)) % 90) * 3.14 / 180)));//+0.7; for turn right
				follower2velocity.angular.z = uslave;;
			}
