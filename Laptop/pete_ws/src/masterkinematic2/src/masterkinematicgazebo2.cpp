...
int main(int argc, char **argv)
{
	...
	ros::Subscriber imu = nh.subscribe("holo/odompy", 1000, handle_allzeta);
	ros::Subscriber degree = nh.subscribe("destinationdegree", 50, handle_degree);

	ros::Publisher follower1velocity_msg = nh.advertise<geometry_msgs::Twist>("non/cmd_vel", 1000);
	...
	{
		ros::spinOnce();
		if (angz > 0.1 && linx != 0 || angz > 0.1 && linx == 0) //> 0.12
		{
			if (startz == 0)
			{
				startz = slavezeta;
			...
			// error
			int eslave = leaderzeta - slavezeta;

			// control signal
			double uslave = (kp * eslave);

			// error2
			int eslave2 = leaderzeta - slave2zeta;

			// control signal2
			float uslave2 = (kp * eslave2);
			...
			follower1velocity.angular.z = signI*3*angz; // 1.37*uleader;
			...
			if ((slavezeta - startz) <= 90 && (slavezeta - startz) >= -10)
			{
			...
			if ((slave2zeta - start2z) <= 90 && (slave2zeta - start2z) >= -10)
			{
			...
			IMUprogram.vector.x = slavezeta ;//goalangleleader;
			IMUprogram.vector.y = startz;//goalangleslave;
			IMUprogram.vector.z = uslave ;//desdegree;
		...
		else if (linx != 0)
		{
			...
			follower1velocity.linear.x = linx;
			follower1velocity.linear.y = 0;
		...
		if (desdegree != 0)
		{
			...
			// control signal
			float uleader = (kp * eleader) + 0.0 * dedtleader + 0.0 * eintegralleader;
