...
double wheelbase = 0.57;                          //Wheelbase, in m
...
void handle_speed( const geometry_msgs::TwistStamped& speed) 
{
  

  speed_act_left = speed.twist.linear.x; //left = linx
  speed_act_right = speed.twist.angular.z; // right = omega
  speed_time = speed.header.stamp;
}

int main(int argc, char** argv){
  ros::init(argc, argv, "odomsteelplate");

  ros::NodeHandle n;
  ros::NodeHandle nh_private_("~");
  ros::Subscriber sub = n.subscribe("odomspeed", 50, handle_speed);
  ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>("metalsheetodom", 50);
  ros::Publisher myodom_pub = n.advertise<geometry_msgs::Pose>("zetaodomsteelplate", 50);
  ...
  double vy = 0.0;
  double vth = 0.0;
  char base_link[] = "/base_steelplate";
  char odom[] = "/odom";
  char kinect[] = "/kinect";
  ...
  while(n.ok()){
    ...
    myodom_msg.orientation.x = theta;
    ...
    vx = (dt == 0)?  0 :  speed_act_left;
    vth = (dt == 0)? 0 :  speed_act_right;
