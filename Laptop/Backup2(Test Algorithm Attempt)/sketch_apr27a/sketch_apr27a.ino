...
// PID Parameters
const double PID_left_param[] = { 0.4, 0, 0 }; //Respectively Kp, Ki and Kd for left motor PID
const double PID_right_param[] = { 0.4, 0, 0 }; //Respectively Kp, Ki and Kd for right motor PID
...
geometry_msgs::Vector3Stamped PWM;                                
ros::Publisher PWM_msg("PWM", &PWM);       

geometry_msgs::Vector3Stamped speedcmd;                                
ros::Publisher speed_cmd("speedcmd", &speedcmd);  
...
void handle_cmd (const geometry_msgs::Twist& cmd_vel) {
  ...
  speedreq.vector.x = speed_req_left;    //left wheel speed (in m/s)
  speedreq.vector.y = speed_req_right;   //right wheel speed (in m/s)
...
void setup() {
  ...
  nh.advertise(speed_cmd);                  //prepare to publish speed in ROS topic
...
void publishSpeed(double time) {
  ...
  speed_msg.vector.x = -1*speed_act_left;    //left wheel speed (in m/s)
  speed_msg.vector.y = -1*speed_act_right;   //right wheel speed (in m/s)
