...
#define Looptime2 3000 //millisec
int sec = 1 ;
#define l 1.697 
#define Pi 3.14159 
#define alpha1 2.514
...
#define d3 0.82
 ...
 double oldangz = 0.0 ;
 double curveslave = 0.0 ;
 double curveslave2 = 0.0 ;
 ...
 void loop()
 {
  ...
  { 
    ...
    if(angz != 0)
    {
      if(angz > 0)
      {
        goalangleslave = goalangleleader + 20 ;
        goalangleslave2 = goalangleleader - 20 ;
        
         //curveslave2 = abs(angz * 0.61) ;
         //curveslave = 0 ;
      }
      else if(angz < 0)
      {
  ...
  // control signal
  float uleader = 0.8*eleader + kd*dedtleader + ki*eintegralleader; 
  ...
  // error2
  int eslave2 = goalangleleader - slave2zeta;
  ...
  follower1velocity.linear.x = 1 ;
  follower1velocity.angular.z = 1 ;
  follower1velocity_msg.publish(&follower1velocity);
  
  /*
  follower2velocity.linear.x = 0.5+0.05 ; linx + angz*R turnleft
  follower2velocity.angular.z = 0.05 ; angz
  follower2velocity_msg.publish(&follower2velocity);
  
  follower3velocity.linear.x = 1+0.05 ; 2linx + angz*R
  follower3velocity.angular.z = 0.05 ; angz
  follower3velocity_msg.publish(&follower3velocity);
    */
  follower2velocity.linear.x = 1-0.05 ; // 2linx +(-angz*R) turnright
  follower2velocity.angular.z = -0.05 ; // -angz
  follower2velocity_msg.publish(&follower2velocity);
  
  follower3velocity.linear.x = 0.5-0.05 ; linx + (-angz*R)
  follower3velocity.angular.z = -0.05 ; // -angz
  follower3velocity_msg.publish(&follower3velocity);
