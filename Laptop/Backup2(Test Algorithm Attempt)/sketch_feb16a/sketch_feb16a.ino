...
const double speed_to_pwm_ratio = 0.00235; //0.0035    // 0.00235 Ratio to convert speed (in m/s) to PWM value. It was obtained by plotting the wheel speed in relation to the PWM motor command (the value is the slope of the linear function).
...
const double max_speed = 3.0;                 //Max speed in m/s
...
PID PID_Motor1(&speed_act_motor1,&speed_cmd_motor1, &speed_req_motor1, 0.25, PID_param[1], 0.01, DIRECT);          //Setting up the PID for left motor
PID PID_Motor2(&speed_act_motor2,&speed_cmd_motor2, &speed_req_motor2, 0.36, PID_param[1], PID_param[2], DIRECT);          //Setting up the PID for left motor
PID PID_Motor3(&speed_act_motor3,&speed_cmd_motor3, &speed_req_motor3, 0.1, PID_param[1], 0.01, DIRECT);          //Setting up the PID for left motor
PID PID_Motor4(&speed_act_motor4,&speed_cmd_motor4, &speed_req_motor4, 0.36, PID_param[1], PID_param[2], DIRECT);          //Setting up the PID for left motor
//motor driver setup
/*
...
ros::Publisher encode_msg("slaveencode", &encode);       

geometry_msgs::Twist speedreq;                                
ros::Publisher speed_reqmsg("slavespeedreq", &speedreq);   

geometry_msgs::Twist PWMout;                                
ros::Publisher PWM_msg("slavePWM", &PWMout);      

geometry_msgs::Vector3Stamped IMUvalue;                                
ros::Publisher IMUvalue_msg("slaveIMU", &IMUvalue);
...
ros::Subscriber<geometry_msgs::Twist> cmd_vel("holo2/cmd_vel",handle_cmd); //cmd_vel  //create a subscriber to ROS topic for velocity commands (will execute "handle_cmd" function when receiving data)
geometry_msgs::TwistStamped speed_msg;                                //create a "speed_msg" ROS message
ros::Publisher speed_pub("slavespeed", &speed_msg);                          //create a publisher to ROS topic "speed" using the "speed_msg" type
...
void loop() {
  ...
  {                                                                           // enter timed loop
    ...
    speed_req_motor1 = (1/7.25)*(linx-liny-((1.8+1.75)*angz));
    speed_req_motor2 = (1/7.25)*(linx+liny+((1.8+1.75)*angz));
    speed_req_motor3 = (1/7.25)*(linx+liny-((1.8+1.75)*angz));
    speed_req_motor4 = (1/7.25)*(linx-liny+((1.8+1.75)*angz));
    ...
    PWM_Motor1 = constrain(((speed_req_motor1+sgn(speed_req_motor1)*min_speed_cmd)/speed_to_pwm_ratio) + (speed_cmd_motor1/speed_to_pwm_ratio), -255, 255); 
    PWM_Motor2 = constrain(((speed_req_motor2+sgn(speed_req_motor2)*min_speed_cmd)/speed_to_pwm_ratio) + (speed_cmd_motor2/speed_to_pwm_ratio), -255, 255);
    PWM_Motor3 = constrain(((speed_req_motor3+sgn(speed_req_motor3)*min_speed_cmd)/speed_to_pwm_ratio) + (speed_cmd_motor3/speed_to_pwm_ratio), -255, 255);
    PWM_Motor4 = constrain(((speed_req_motor4+sgn(speed_req_motor4)*min_speed_cmd)/speed_to_pwm_ratio) + (speed_cmd_motor4/speed_to_pwm_ratio), -255, 255);
    ...
    PWMout.linear.x = PWM1 ;
    PWMout.linear.y = PWM2 ;
    PWMout.angular.x = PWM3 ;
    PWMout.angular.y = PWM4 ;

    if(dirreal1 == 1) // forward left
    {
      motor1.run("FORWARD");
    }
    else if(dirreal1 == 2)
...
void publishSpeed(double time) 
{
  ...
  speed_msg.twist.angular.x = speed_act_motor3;    //left wheel speed (in m/s)
  speed_msg.twist.angular.y = speed_act_motor4;   //right wheel speed (in m/s)  
