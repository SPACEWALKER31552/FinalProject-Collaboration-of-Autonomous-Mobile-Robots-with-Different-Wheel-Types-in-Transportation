...
void setup() 
 ...
 void loop() {
   ...
   if((millis()-lastmillis) >= Looptime)
    ...
    // slave --------------------------------------------------------------------------------------
    if((leaderzeta - startz-26.19)>0)
     {
      slavediff = leaderzeta - startz - 26.19;
     ...
     else if((leaderzeta - startz-26.19)<0)
     {
     slavediff = leaderzeta - startz - 26.19;
     //Serial.println(slavediff);
     if(slavediff<-360)
    ...
    follower1velocity.linear.x = abs(angz*2.5); //1.7
    follower1velocity.linear.y = 0;
    follower1velocity.angular.z = angz*1.1;
    /*
    //slave---------------------------------------------------------
    ...
    if((leaderzeta - startz)<-270 && (leaderzeta - startz)>=-360 ) 
    {
    follower1velocity.linear.x = linx + (slaveturnicomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) - (slaveturnjcomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    //follower1velocity.linear.y = -(slaveturnjcomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) + (slaveturnicomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    follower1velocity.angular.z = angz/10;
    }
    else if((leaderzeta - startz)<-180 && (leaderzeta - startz)>=-270 ) 
    {
    follower1velocity.linear.x = linx + (slaveturnjcomponent*abs(cos((int(abs(slavezeta - startz))%90)*3.14/180))) - (slaveturnicomponent*abs(sin((int(abs(slavezeta - startz))%90)*3.14/180)));
    //follower1velocity.linear.y = +(slaveturnicomponent*abs(cos((int(abs(slavezeta - startz))%90)*3.14/180))) + (slaveturnjcomponent*abs(sin((int(abs(slavezeta - startz))%90)*3.14/180)));
    follower1velocity.angular.z = angz/10;
    }
    else if((leaderzeta - startz)<-90 && (leaderzeta - startz)>=-180 ) 
    {
    follower1velocity.linear.x = linx + (slaveturnicomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) - (slaveturnjcomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    //follower1velocity.linear.y = +(slaveturnjcomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) + (slaveturnicomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    follower1velocity.angular.z = angz/10;
    }
    else if((leaderzeta - startz)<0 && (leaderzeta - startz)>=-90 ) 
    {
    follower1velocity.linear.x = linx - (slaveturnjcomponent*abs(cos((int(abs(slavezeta - startz))%90)*3.14/180))) - (slaveturnicomponent*abs(sin((int(abs(slavezeta - startz))%90)*3.14/180)));
    //follower1velocity.linear.y = -(slaveturnicomponent*abs(cos((int(abs(slavezeta - startz))%90)*3.14/180))) + (slaveturnjcomponent*abs(sin((int(abs(slavezeta - startz))%90)*3.14/180)));
    follower1velocity.angular.z = angz/10;
    ...
    else if((leaderzeta - startz)<=90 && (leaderzeta - startz)>=0 ) 
    {
    follower1velocity.linear.x = linx + (slaveturnjcomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) - (slaveturnicomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    //follower1velocity.linear.y = -(slaveturnicomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) - (slaveturnjcomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    follower1velocity.angular.z = angz/10;
    }
    else if((leaderzeta - startz)<= 180 && (leaderzeta - startz)>90 )
    {
    follower1velocity.linear.x = linx + (slaveturnicomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) - (slaveturnjcomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    //follower1velocity.linear.y = -(slaveturnjcomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) + (slaveturnicomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    follower1velocity.angular.z = angz/10;
    }
    else if((leaderzeta - startz)<=270 && (leaderzeta - startz)>180 )
    {
    follower1velocity.linear.x = linx + (slaveturnjcomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) - (slaveturnicomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    ...
    else if((leaderzeta - startz)<= 360 && (leaderzeta - startz)>270)
    {
    follower1velocity.linear.x = linx + (slaveturnicomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) - (slaveturnjcomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    //follower1velocity.linear.y = (slaveturnjcomponent*abs(cos((int((slavezeta - startz))%90)*3.14/180))) + (slaveturnicomponent*abs(sin((int((slavezeta - startz))%90)*3.14/180)));
    follower1velocity.angular.z = angz/10;
