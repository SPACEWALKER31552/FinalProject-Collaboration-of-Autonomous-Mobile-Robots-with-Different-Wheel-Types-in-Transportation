   ...
   if((millis()-lastmillis) >= Looptime)
    {
     ...
     if((slave2zeta - start2z)>0) //19.45
     {
      slave2diff = slave2zeta - start2z;
    ...
    if((slave2zeta - start2z)<=90 && (slave2zeta - start2z)>=0 ) 
    {
    ...
    follower1velocity.linear.x = 1.2*linx ;
